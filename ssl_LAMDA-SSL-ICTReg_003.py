...
unlabeled_X, labeled_X, unlabeled_y, labeled_y = train_test_split(x_train, y_train, test_size=0.2)
...
file = open("./LAMDA-SSL-ICTReg3.txt", "w")

model=ICTReg(alpha=0.5,lambda_u=0.001,warmup=1/32,
...
model.fit(X=labeled_X,y=labeled_y,unlabeled_X=unlabeled_X,valid_X=x_val,valid_y=y_val)
