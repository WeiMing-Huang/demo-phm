...
from simsiam_ab import SimSiam
import random
import torch
...
def train_loop(model, which_model, train_epochs, train_dataloader, train_loss_fn, train_optimizer, train_scheduler, train_patients):
    ...
    for epoch in range(train_epochs):
        ...
        for idx, (inputs, labels) in enumerate(train_dataloader):
            ...
            z1, z2, p1, p2 = model(inputs1.float(), inputs3.float())

            loss = train_loss_fn(z1, z2, p1, p2)
...
def objective(trial):
    ...
    train_loop(model=model, 
                ...
                train_loss_fn=utils.TwoBrenchLoss, 
...
# row = {'model':experiment,'RMSE_m':np.mean(save_rmse), 'RMSE_s':np.std(save_rmse), 'Score_m':np.mean(save_res), 'Score_s':np.std(save_res)}
