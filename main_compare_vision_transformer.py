...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    model = SimSiam(backbone=ViT(in_channels=1, patchx_size=6, patchy_size=x_train.shape[-1], emb_size=768, num_heads=16, dropout=0.1, imgx_size=config.sequence_length, imgy_size=x_train.shape[-1], depth=3, n_classes=128,), backbone_out=128)
