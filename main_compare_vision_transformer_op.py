...
def objective(trial):
    ...
    hyperparams = {
        ...
        'patchy_size': x_train.shape[-1],
        'emb_size': trial.suggest_categorical('emb_size', [256, 512, 768]),
        'num_heads': trial.suggest_categorical('num_heads', [4, 8, 16]),
        'dropout': trial.suggest_float('dropout', 0, 0.5, step=0.1),
        'imgx_size': config.sequence_length,
        'imgy_size': x_train.shape[-1],
        'depth': trial.suggest_int('depth', 2, 8),
        'n_classes': 128,
    }

    model = SimSiam(backbone=ViT(**hyperparams), backbone_out=128).to(device)
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    model = SimSiam(backbone=ViT(**hyperparams), backbone_out=128)
