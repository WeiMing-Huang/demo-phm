...
def objective(trial):
    ...
    model = SimSiam(backbone=LSTM(**hyperparams), backbone_out=128).to(device)
...
hyperparams = {
    ...
    'layer_dim': study.best_trial.params['layer_dim'],
    'output_dim': 128,
}
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    model = SimSiam(backbone=LSTM(**hyperparams), backbone_out=128)
    ...
    test_loop(model_all=model, model_final=model_final,
              which_model=1, x_test=x_test, y_test=y_test)
