...
import argparse
import optuna
from compare_bi_lstm_attention import AttBiLSTM
...
experiment = f'{config.dataset}_{config.modelname}_optuna_model_best_cv_dict_{datetime.now().today().strftime("%m_%d_%H_%M_%S")}'
...
def objective(trial):
    ...
    x_train_op, x_test_op, y_train_op, y_test_op  = train_test_split(x_train, y_train, test_size=config.label_size)
    ...
    unlabel_op_loader = DataLoader(train_op_dataset, batch_size=config.batch, drop_last=True)
    label_op_loader = DataLoader(test_op_dataset, batch_size=config.batch, drop_last=True)
    val_op_loader = DataLoader(val_op_dataset, batch_size=config.batch, drop_last=True)

    hyperparams = {
        'n_classes': 128,
        'emb_size': x_train.shape[-1],
        'rnn_size': trial.suggest_categorical('rnn_size', [128, 256, 512]),
        'rnn_layers': trial.suggest_int('rnn_layers', 1, 5),
        'dropout': trial.suggest_float('dropout', 0.2, 0.8, step=0.1),
    }

    model = SimSiam(backbone=AttBiLSTM(**hyperparams), backbone_out=128).to(device)

    train_optimizer = torch.optim.SGD(model.parameters(), lr=trial.suggest_categorical('lr', [0.5, 0.1, 0.05]), momentum=0.9, nesterov=True, weight_decay=0.01)
    ...
    with torch.no_grad():
        ...
        if which_model == 1:
            preds = model_final(torch.from_numpy(x_val).to(device).float()).detach().cpu().numpy()
        elif which_model == 2:
            preds = model_final(torch.from_numpy(x_val).unsqueeze(1).to(device).float()).detach().cpu().numpy()
        else:
            pass
...
hyperparams = {
    'n_classes': 128,
    'emb_size': x_train.shape[-1],
    'rnn_size': study.best_trial.params['rnn_size'],
    'rnn_layers': study.best_trial.params['rnn_layers'],
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    model = SimSiam(backbone=AttBiLSTM(**hyperparams), backbone_out=128)

    model = model.to(device)

    train_optimizer = torch.optim.SGD(model.parameters(), lr=study.best_trial.params['lr'], momentum=0.9, nesterov=True, weight_decay=0.01)
    ...
    test_loop(model_all=model, model_final=model_final, which_model=1, x_test=x_test, y_test=y_test)
...
row = {'model':experiment,'RMSE_m':np.mean(save_rmse), 'RMSE_s':np.std(save_rmse), 'Score_m':np.mean(save_res), 'Score_s':np.std(save_res), 'RMSE_all':save_rmse, 'Score_all':save_res}
