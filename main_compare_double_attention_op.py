...
import argparse
import optuna
from compare_double_attention import DoubleAttentionLayer
...
def objective(trial):
    ...
    hyperparams = {
        'in_channels': 1,
        'c_m': trial.suggest_categorical('c_m', [128, 256, 512]),
        'c_n': trial.suggest_categorical('c_n', [128, 256, 512]),
        'output_dim': 128,
    }

    model = SimSiam(backbone=DoubleAttentionLayer(**hyperparams), backbone_out=128).to(device)
...
hyperparams = {
    'in_channels': 1,
    'c_m': study.best_trial.params['c_m'],
    'c_n': study.best_trial.params['c_n'],
    'output_dim': 128,
}
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    model = SimSiam(backbone=DoubleAttentionLayer(**hyperparams), backbone_out=128)
