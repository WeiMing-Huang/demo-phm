...
from compare_timeseries_transformer import TimeSeriesTransformer
...
experiment = f'{config.dataset}_{config.modelname}_model_best_cv_dict_{datetime.now().today().strftime("%m_%d_%H_%M_%S")}'
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    model = SimSiam(backbone=TimeSeriesTransformer(input_size=x_train.shape[-1], dec_seq_len=config.sequence_length, batch_first=True, num_predicted_features=128), backbone_out=128)
